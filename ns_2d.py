...
def navier_stokes_2d(w0, f, visc, T, delta_t=1e-4, record_steps=1):
    ...
    for j in range(steps):
        ...
        q = 2.0 * math.pi * k_y * 1j * psi_h
        q = torch.fft.irfft2(q, s=(N, N))

        # Velocity field in y-direction = -psi_x
        v = -2.0 * math.pi * k_x * 1j * psi_h
        v = torch.fft.irfft2(v, s=(N, N))

        # Partial x of vorticity
        w_x = 2.0 * math.pi * k_x * 1j * w_h
        w_x = torch.fft.irfft2(w_x, s=(N, N))

        # Partial y of vorticity
        w_y = 2.0 * math.pi * k_y * 1j * w_h
...
for j in range(N // bsize):
    ...
    # Solve NS
    sol, sol_t = navier_stokes_2d(w0, f, float(sys.argv[1]), 50.0, 1e-4, record_steps)
    ...
    c += bsize
    t1 = default_timer()
    print(j, c, t1 - t0, flush=True)

scipy.io.savemat(
    f"./ns_data_V{sys.argv[1]}_N1000_{sys.argv[2]}_T50.mat",
