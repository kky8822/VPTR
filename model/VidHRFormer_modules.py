...
class VidHRFormerBlockEnc(nn.Module):
    def __init__(
        ...
        self.SLMHSA = SpatialLocalMultiheadAttention(
            embed_dim, num_heads, window_size, dropout, rpe
        )
        ...
        self.temporal_MHSA = nn.MultiheadAttention(
    ...
    def forward(self, x, local_window_pos_embed, temporal_pos_embed):
        ...
        def temporal_self_attention(q, temporal_pos_embed, attn_mask=None):
            q1 = self.norm3(q)
            if attn_mask is not None:
                out = self.drop1(
                    self.temporal_MHSA(
                        q1 + temporal_pos_embed[:, None, :],
                        q1 + temporal_pos_embed[:, None, :],
                        q1,
        ...
        N, T, H, W, C = x.shape
        # print("VidHRFormer start: ", x.shape)
        x = x + self.drop_path(
            self.SLMHSA(self.norm1(x), local_window_pos_embed)
        )  # spatial local window self-attention, and skip connection
        # print("SPMHSA: ", x.shape)
        # Conv feed-forward, different local window information interacts
        x = x + self.drop_path(self.SpatialFFN(self.norm2(x)))  # (N, T, H, W, C)
        # print("SpaitalFFN: ", x.shape)
        ...
        if self.far:
            ...
            if self.rk == False:
                F1 = temporal_self_attention(
                    q=x, temporal_pos_embed=temporal_pos_embed, attn_mask=attn_mask
                )
                x = x + F1
            elif self.rk == "rk2":
                F1 = temporal_self_attention(
                ...
                F2 = temporal_self_attention(
                    q=x + F1, temporal_pos_embed=temporal_pos_embed, attn_mask=attn_mask
                )
                x = x + 1 / 2 * (F1 + F2)
            elif self.rk == "rk4":
                F1 = temporal_self_attention(
                ...
                F3 = temporal_self_attention(
                    q=x + 1 / 2 * F2,
                    temporal_pos_embed=temporal_pos_embed,
                    attn_mask=attn_mask,
                )
                F4 = temporal_self_attention(
                    q=x + F3, temporal_pos_embed=temporal_pos_embed, attn_mask=attn_mask
                )
        ...
        x = x + self.drop3(x1)
        # print("MLP FFN: ", x.shape)

        x = x.reshape(T, N, H, W, C).permute(1, 0, 2, 3, 4)
        # print("VidHRFormer Fin: ", x.shape)
        return x
...
class VidHRformerDecoderNAR(nn.Module):
    ...
    def forward(
        ...
        query_pos,
        memory,
        local_window_pos_embed,
        future_query_temporal_pos_embed,
        TS_local_pos_embed,
        past_query_temporal_pos_embed,
    ):
        output = tgt
...
class VidHRFormerBlockDecNAR(nn.Module):
    def __init__(
        ...
        if TSLMA_flag:
            self.TSLMA = TemporalSpatialLocalMultiheadAttention(
                embed_dim, num_heads, window_size, dropout
            )
        else:
            self.EncDecAttn = nn.MultiheadAttention(
                self.embed_dim, self.num_heads, dropout=dropout
            )
        self.SpatialFFN1 = MlpDWBN(
            embed_dim,
            hidden_features=int(Spatial_FFN_hidden_ratio * embed_dim),
            out_features=embed_dim,
            drop=dropout,
    ...
    def forward(
        ...
        def temporal_enc_dec_attention(
            q,
            k,
            query_pos,
            future_query_temporal_pos_embed,
            ...
            q = self.norm5(q)
            T1 = k.shape[1]
            k = k.permute(1, 0, 2, 3, 4).reshape(T1, N * H * W, C)
            query_pos = query_pos.permute(1, 0, 2, 3, 4).reshape(T2, N * H * W, C)
            out = self.drop_path1(
                self.EncDecAttn(
                    query=q + query_pos + future_query_temporal_pos_embed[:, None, :],
                    key=k + past_query_temporal_pos_embed[:, None, :],
                    value=k,
                )[0]
            )
            return out
        ...
        tgt2 = tgt + self.drop_path(
            self.SLMHSA(tgt2_query_pos, local_window_pos_embed, value=tgt2)
        )  # spatial local window self-attention, and skip connection
        # Conv feed-forward, different local window information interacts
        tgt2 = tgt2 + self.drop_path(
            self.SpatialFFN(self.norm2(tgt2))
        )  # (N, T, H, W, C)

        # query temporal self-attention
        tgt2 = tgt2.permute(1, 0, 2, 3, 4).reshape(T2, N * H * W, C)
        tgt = self.norm3(tgt2)
        tgt2 = tgt2 + self.drop1(
            self.temporal_MHSA(
        ...
        if self.TSLMA_flag:
            ...
            tgt2 = tgt2 + self.drop_path1(
                self.TSLMA(memory, tgt + query_pos, TS_local_pos_embed)
            )
        else:
            if self.rk == False:
                F1 = temporal_enc_dec_attention(
                    q=tgt2,
                    k=memory,
                    query_pos=query_pos,
                    future_query_temporal_pos_embed=future_query_temporal_pos_embed,
                    past_query_temporal_pos_embed=past_query_temporal_pos_embed,
                )
                tgt2 = tgt2 + F1
            elif self.rk == "rk2":
                F1 = temporal_enc_dec_attention(
...
class TemporalSpatialLocalMultiheadAttention(nn.Module):
    ...
    def forward(self, memory, query, TS_local_pos_embed):
        ...
        # permute
        memory_permute = self.permute_helper.permute(
            memory_pad, memory_pad.size()
        )  # (T1*window_size*window_size, N*H/window_size*W/window_size, C)
        query_permute = self.permute_helper.permute(
            query_pad, query_pad.size()
        )  # (T2*window_size*window_size, N*H/window_size*W/window_size, C)

        # attention
        out = self.attn(
            query=query_permute
            + TS_local_pos_embed[T1 : T1 + T2, ...].flatten(0, 2)[:, None, :],
            key=memory_permute
            + TS_local_pos_embed[0:T1, ...].flatten(0, 2)[:, None, :],
            value=memory_permute,
        )[
            0
        ]  # (T2*window_size*window_size, N*H/window_size*W/window_size, C)
        # reverse permutation
...
class SpatialLocalMultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, window_size=7, dropout=0.0, rpe=False):
        super().__init__()
    ...
    def forward(self, x, local_pos_embed, value=None):
        ...
        # permute
        x_permute = self.permute_helper.permute(
            x_pad, x_pad.size()
        )  # (window_size*window_size, N*T*H/window_size*W/window_size, C)
        ...
        if value is not None:
            ...
            value_permute = self.permute_helper.permute(
                value_pad, value_pad.size()
            )  # (window_size*window_size, N*T*H/window_size*W/window_size, C)
            # attention
