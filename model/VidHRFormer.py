...
class VidHRFormerNAR(nn.Module):
    def __init__(
        ...
        self.decoder = VidHRformerDecoderNAR(
            VidHRFormerBlockDecNAR(
                embed_dim,
                num_heads,
                window_size,
                dropout,
    ...
    def forward(
        ...
        memory = self.encoder(
            src, local_window_pos_embed, temporal_pos_embed[0:Tp, ...]
        )
        ...
        out = self.decoder(
            ...
            memory,
            local_window_pos_embed,
            temporal_pos_embed[Tp:, ...],
            TS_local_pos_embed,
            temporal_pos_embed[0:Tp, ...],
        )  # (N, Tf, H, W, embed_dim)
