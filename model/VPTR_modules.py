...
from utils.position_encoding import (
    PositionEmbeddding1D,
    PositionEmbeddding2D,
    PositionEmbeddding3D,
)
import functools
...
class VPTRFormerNAR(nn.Module):
    def __init__(
        ...
        self.transformer = VidHRFormerNAR(
            (d_model, encH, encW),
            num_encoder_layers,
            num_decoder_layers,
            num_past_frames,
        ...
        Tlw_pos = pos3d(NestedTensor(torch.empty(T * 1, self.d_model, window_size, window_size), None))[0, ...].permute(
            1, 2, 3, 0
        )
...
class VPTRFormerFAR(nn.Module):
    def __init__(
        ...
        pos1d = PositionEmbeddding1D()
        temporal_pos = pos1d(L=T, N=1, E=self.d_model)[:, 0, :]
        # print("temporal_emb: ", temporal_pos.shape)
        self.register_buffer("temporal_pos", temporal_pos)

        pos2d = PositionEmbeddding2D(device=self.device)
        lw_pos = pos2d(N=1, E=self.d_model, H=window_size, W=window_size)[0, ...].permute(1, 2, 0)
        # print("spatial_emb: ", lw_pos.shape)
        self.register_buffer("lw_pos", lw_pos)
