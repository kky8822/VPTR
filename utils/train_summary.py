...
def resume_training(module_dict, optimizer_dict, resume_ckpt, loss_name_list=None, map_location=None):
    ...
    (
        ...
        start_epoch,
        history_loss_dict,
    ) = load_ckpt(resume_ckpt, map_location)
...
def save_ckpt(Modules_dict, Optimizers_dict, epoch, loss_dict, save_dir):
    ...
    for k, m in Modules_dict.items():
        ...
        # print("module_state_dict: ", k, module_state_dict[k].shape)
    optim_state_dict = {}
    for k, m in Optimizers_dict.items():
        optim_state_dict[k] = m.state_dict()
        # print("optim_state_dict: ", k, optim_state_dict[k].shape)
...
def load_ckpt(ckpt_file, map_location=None):
    ...
    return Modules_state_dict, Optimizers_state_dict, epoch, loss_dict


def visualize_batch_clips(
    gt_past_frames_batch,
    gt_future_frames_batch,
    pred_frames_batch,
    file_dir,
    ...
    def save_clip(clip, file_name):
        ...
        # print("clip", torch.min(clip), torch.max(clip))
    ...
    gt_frames_batch = torch.cat((gt_past_frames_batch, gt_future_frames_batch), dim=1)

    max_length = max(gt_frames_batch.shape[1], pred_frames_batch.shape[1])
    if gt_frames_batch.shape[1] < max_length:
        gt_frames_batch = append_frames(gt_frames_batch, max_length)
    if pred_frames_batch.shape[1] < max_length:
        pred_frames_batch = append_frames(pred_frames_batch, max_length)

    batch = torch.cat([gt_frames_batch, pred_frames_batch], dim=-1)  # shape (N, clip_length, C, H, 3W)
    ...
    # N,t,c,x,y
    N, T, C, X, Y = batch.shape
    # minV = minV.expand(N, T, X, Y, C).permute(0, 1, 4, 2, 3)
    # maxV = maxV.expand(N, T, X, Y, C).permute(0, 1, 4, 2, 3)
    # batch = (batch - minV) / (maxV - minV) * 1.0
    for n in range(N):
        tg = batch[n]
        if renorm_transform is not None:
            # print("before: ", torch.min(clip), torch.max(clip))
            # print(n, "raw", torch.min(tg[:, 0, ...]), torch.max(tg[:, 0, ...]))
            tg = renorm_transform(tg)
            # print(n, "renorm", torch.min(tg[:, 0, ...]), torch.max(tg[:, 0, ...]))

            # minV = torch.Tensor(
            #     [-7.45e00, -1.41e-01, -7.45e00, -1.21e02, -1.26e02, -5.68e03, -4.00e00]
            # )
            # maxV = torch.Tensor(
            #     [7.66e00, 1.41e-01, 7.66e00, 1.20e02, 1.15e02, 5.59e03, -2.00e00]
            # )
            # minV = minV.expand(T, X, Y, C).permute(0, 3, 1, 2)
            # maxV = maxV.expand(T, X, Y, C).permute(0, 3, 1, 2)
            # tg = (tg - minV) / (maxV - minV)
            # print(n, "minmax", torch.min(tg[:, 0, ...]), torch.max(tg[:, 0, ...]))

        for c in range(C):
            if c == 0:
                clip = tg[:, c, ...].unsqueeze(1)
            else:
                clip = torch.cat((clip, tg[:, c, ...].unsqueeze(1)), dim=2)

        file_name = file_dir.joinpath(f"{desc}_clip_{n}.gif")
        save_clip(clip, file_name)


def visualize_batch_clips_inference(
    idx,
    gt_past_frames_batch,
    gt_future_frames_batch,
    pred_frames_batch,
    ...
    for n in range(N):
        ...
        file_name = file_dir.joinpath(f"{desc}_clip_{idx*N+n}.gif")
        save_clip(clip, file_name)
