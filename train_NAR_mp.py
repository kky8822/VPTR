...
def NAR_show_samples(
    VPTR_Enc, VPTR_Dec, VPTR_Transformer, sample, save_dir, device, renorm_transform
):
...
def init_models(
    ...
    VPTR_Dec = VPTRDec(
        img_channels, feat_dim=encC, n_downsampling=3, out_layer=out_layer
    ).to(rank)
    ...
    bpnce = BiPatchNCE(batch_size // world_size, num_future_frames, encH, encW, 1.0).to(
        rank
    )
...
def single_iter(
    ...
    if train_flag:
        ...
        pred_future_feats = VPTR_Transformer.module.NCE_projector(
            pred_future_feats.permute(0, 1, 3, 4, 2)
        ).permute(0, 1, 4, 2, 3)
        future_gt_feats = VPTR_Transformer.module.NCE_projector(
...
if __name__ == "__main__":
    ...
    ckpt_save_dir = Path(
        "/home/travail/xiyex/VPTR_ckpts/MNIST_TLMA_aug_BPNCE01_MSEGDL_NAR_mp_ckpt"
    )
    tensorboard_save_dir = Path(
        "/home/travail/xiyex/VPTR_ckpts/MNIST_TLMA_aug_BPNCE01_MSEGDL_NAR_mp_tensorboard"
    )
    resume_AE_ckpt = Path(
        "/home/travail/xiyex/VPTR_ckpts/MNIST_ResNetAE_MSEGDLgan001_ckpt"
    ).joinpath("epoch_93.tar")
